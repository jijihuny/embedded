...
void MPU9250_init(uint8_t Ascale, uint8_t Gscale, uint8_t sampleRate){
    ...
    SPI0_write(PWR_MGMT_1, 0x01);//Auto select clock source to be PLL gyroscope reference if ready else
    delay(200);
    ...
    SPI0_write(INT_PIN_CFG, 0x10);//INT is 50 microsecond pulse and any read to clear
...
void calibrate_sensor(float* accel, float* gyro){
    ...
    while(SPI0_read(PWR_MGMT_1) & (uint8_t) 0x80);//wait for device completely be reseted
    ...
    SPI0_write(USER_CTRL, 0x04);//Reset FIFO
    delay(15);
    ...
    for(int i=0; i<3; i++){//Divide by 4 to get 32.9 LSB per deg/s to conform to expected bias input format
